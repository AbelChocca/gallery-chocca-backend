...
class SagaService:
    ...
    async def execute_step(
        ...
        result = await self.execute_last()

        if compensation_factory:
            compensation, name, kwargs = compensation_factory(result)

            step.compensation = compensation
            step.compensation_name = name
            step.compensation_kwargs = kwargs
