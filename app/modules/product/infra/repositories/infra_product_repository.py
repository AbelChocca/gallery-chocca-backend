...
from app.shared.exceptions.domain.product_exception import ProductNotFound
from app.shared.exceptions.domain.variant_color_exception import VariantProductNotFound, CannotDeleteVariantProduct
from app.shared.exceptions.domain.image_exception import ImageNotFound, CannotDeleteImage
...
class PostgresProductRepository(ProductRepository):
    ...
    async def delete_by_id(self, id: int) -> None:
        try:
            ...
            await self.db_session.rollback()
            raise DatabaseException("Cannot delete the product from database") from e
    ...
    async def delete_image_by_id(self, cloudinary_id: str) -> None:
        ...
        if not image:
            raise ImageNotFound(f"Image with id:{cloudinary_id} was not found.")
        ...
        if len(variant.imagenes) <= 1:
            raise CannotDeleteImage("Cannot delete image from database, need atleast two images to delete one")
        try:
